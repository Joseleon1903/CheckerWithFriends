...
public class WsClient
{
    ...
    public async Task Connect()
    {
        ...
        LoggerFile.Instance.DEBUG_LINE("Connecting to: " + serverUri);
        await ws.ConnectAsync(serverUri, CancellationToken.None);
        while (IsConnecting())
        {
            Debug.Log("Waiting to connect...");
            LoggerFile.Instance.DEBUG_LINE("Waiting to connect...");
            Task.Delay(50).Wait();
        }
    ...
    private async void RunReceive()
    {
        ...
        LoggerFile.Instance.DEBUG_LINE("WebSocket Message Receiver looping.");
        string result;
        while (true)
        {
